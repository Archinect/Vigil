...
obj/item/weapon/gun/energy/freezegun/Destroy()
...
/obj/structure/freezedmob/process() // soooo uglycode!
	...
	if(occupant.stat != 2)
		...
		occupant.apply_effect(2 , WEAKEN)
...
/*	var/icon/mobpic = getFlatIcon(M, I.dir) //damn, not working. GOD WHY!?
	...
	if(M.client)
		...
		M.client.eye = I
	M.loc = I
	//M.apply_effect(10 , WEAKEN) //dunno how make it true or falce, but mob shouldnt use hist equip while freezed
	I.occupant = M
	return 1
