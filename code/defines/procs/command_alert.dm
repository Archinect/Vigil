...
/proc/command_alert(var/text, var/title = "",var/force_report = 0,var/alert,var/noalert = 0)
	...
	if (title && length(title) > 0)
		command += "<br><h2 class='alert'>[gibberish ? Gibberish(html_encode(title),70) : rhtml_encode(title)]</h2>"


	command += {"<br><span class='alert'>[gibberish_main ? Gibberish(html_encode(text),70) : rhtml_encode(text)]</span><br>
		<br>"}
