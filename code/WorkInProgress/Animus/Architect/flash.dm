/obj/item/device/flash
	...
	var/obj/item/weapon/cell/bcell = null //Батарейка во флэшке
	var/hitcost = 50	//Стоимость разового использования флэшки в энергии

/obj/item/device/flash/New()	//Батарейка предустанавливается
	..()
	bcell = new(src)
	bcell.charge=bcell.maxcharge // Charge this shit
	return

/obj/item/device/flash/attackby(obj/item/weapon/W, mob/user, params)
	...
	else if(isscrewdriver(W))	//Если тыкают отверткой, достает батарейку.
		if(bcell)
			bcell.updateicon()
...
/obj/item/device/flash/examine(mob/user) //Количество заряда во флэшке при экзамайне
	..(user)
	if(bcell)
		to_chat(user, "<span class='notice'>The flash is [round(bcell.percent())]% charged.</span>")
	if(!bcell)
		to_chat(user, "<span class='warning'>The flash does not have a power source installed.</span>")
...
/obj/item/device/flash/proc/flash_recharge() //Черная магия
...
/obj/item/device/flash/proc/try_use_flash(var/mob/user = null) //Проверки на работоспособность флэшки и человека
	flash_recharge(user)

	if(broken)
		return 0
	if(bcell.charge < hitcost)
		visible_message("<span class='notice'>The [src.name] is not enough energy</span>")
		return 0
