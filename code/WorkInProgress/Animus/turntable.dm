...
/obj/machinery/party/turntable/attack_hand(mob/living/user as mob)
	...
	t += "<A href='?src=\ref[src];off=1'><font color='maroon'>T</font><font color='green'>urn</font> <font color='red'>Off</font></A>"
		...
		if(src.playing == 0)
			...
			while(playing == 1)
				for(var/mob/M in world)
					...
					if((location in RA) && M.music == 0)
						//world << "Found the song..."
						M << S
						M.music = 1
					else if(!(location in RA) && M.music == 1)
...
/obj/machinery/party/gramophone/attack_hand(mob/living/user as mob)
	...
	if (src.playing == 0)
		...
		var/sound/S
		S = sound(pick('sound/turntable/Taintedlove.ogg','sound/turntable/Soviet.ogg'))
		S.repeat = 1
		S.channel = 10
		S.falloff = 2
		S.wait = 1
		...
		for(RA)
			...
			while(playing == 1)
				for(var/mob/M in world)
					if((M.loc in RA) && M.music == 0)
						M << S
						M.music = 1
					else if(!(M.loc in RA) && M.music == 1)
						var/sound/Soff = sound(null)
						Soff.channel = 10
						M << Soff
						M.music = 0
