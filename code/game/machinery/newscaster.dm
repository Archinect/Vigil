	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon) || isobserver(usr)))
		...
		if(href_list["set_channel_name"])
			if(isobserver(usr) && !canGhostWrite(usr,src,"set a channel's name"))
				...
				return
			src.channel_name = sanitize(stripped_input(usr, "Provide a Feed Channel Name", "Network Channel Handler", ""))
			...
			if(isnull(src.msg))
				src.msg = ""
			src.msg = trim(sanitize(stripped_multiline_input(usr, "Write your Feed story", "Network Channel Handler", src.msg)))
			...
			if(isobserver(usr) && !canGhostWrite(usr,src,"tried to set the description of a wanted person"))
				to_chat(usr, "<span class='warning'>You can't do that.</span>")
				return
			src.msg = trim(sanitize(stripped_input(usr, "Provide the a description of the Wanted person and any other details you deem important", "Network Security Handler"), 1))
			src.updateUsrDialog()
...
obj/item/weapon/newspaper/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/pen))
		if(src.scribble_page == src.curr_page)
			...
			var/s = sanitize(stripped_input(user, "Write something", "Newspaper", "") )
