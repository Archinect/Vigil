		...
		if (src.authenticated)
				...
				switch(href_list["field"])
					...
					if("mi_dis")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please input minor disabilities list:", "Med. records", src.active2.fields["mi_dis"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["mi_dis"] = t1
					if("mi_dis_d")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please summarize minor dis.:", "Med. records", src.active2.fields["mi_dis_d"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["mi_dis_d"] = t1
					if("ma_dis")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please input major diabilities list:", "Med. records", src.active2.fields["ma_dis"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["ma_dis"] = t1
					if("ma_dis_d")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please summarize major dis.:", "Med. records", src.active2.fields["ma_dis_d"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["ma_dis_d"] = t1
					if("alg")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please state allergies:", "Med. records", src.active2.fields["alg"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["alg"] = t1
					if("alg_d")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please summarize allergies:", "Med. records", src.active2.fields["alg_d"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["alg_d"] = t1
					if("cdi")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please state diseases:", "Med. records", src.active2.fields["cdi"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["cdi"] = t1
					if("cdi_d")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please summarize diseases:", "Med. records", src.active2.fields["cdi_d"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
							src.active2.fields["cdi_d"] = t1
					if("notes")
						if (istype(src.active2, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please summarize notes:", "Med. records", src.active2.fields["notes"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
								return
					...
					if("b_dna")
						if (istype(src.active1, /datum/data/record))
							var/t1 = sanitize(stripped_input("Please input DNA hash:", "Med. records", src.active1.fields["dna"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active1 != a1))
								return
					...
					if("vir_name")
						...
						if (v)
							var/t1 = sanitize(stripped_input("Please input pathogen name:", "VirusDB", v.fields["name"], null), 1)
							if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active1 != a1))
								return
							v.fields["name"] = t1
					if("vir_desc")
			...
			if (href_list["add_c"])
				if (!( istype(src.active2, /datum/data/record) ))
					return
				var/a2 = src.active2
				var/t1 = sanitize(stripped_multiline_input("Add Comment:", "Med. records", null, null), 1)
				if ((!( t1 ) || !( src.authenticated ) || usr.stat || usr.restrained() || (!in_range(src, usr) && (!istype(usr, /mob/living/silicon))) || src.active2 != a2))
					return
				var/counter = 1
				while(src.active2.fields[text("com_[]", counter)])
			...
			if (href_list["search"])
				...
				t1 = lowertext_alt(t1)
				for(var/datum/data/record/R in data_core.medical)
					if ((lowertext_alt(R.fields["name"]) == t1 || t1 == lowertext_alt(R.fields["id"]) || t1 == lowertext_alt(R.fields["b_dna"])))
						src.active2 = R
					else
