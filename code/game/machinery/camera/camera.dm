...
//		to_chat(user, "<span class='notice'>You start to [panel_open ? "close" : "open"] the camera's panel.</span>")
	...
	// OTHER
		...
		camera_messages["[rhtml_encode(W.name)]"] = info

		for(var/mob/living/silicon/ai/O in living_mob_list)
			if(!O.client)
				continue
			if(U.name == "Unknown")
				to_chat( O, "<span class='name'>[U]</span> holds a <a href='byond://?src=\ref[src];picturename=[rhtml_encode(W.name)]'>[W]</a> up to one of your cameras ...")
			else
				to_chat(O, "<span class='name'><a href='byond://?src=\ref[O];track2=\ref[O];track=\ref[U]'>[U]</a></span> holds a <a href='byond://?src=\ref[src];picturename=[rhtml_encode(W.name)]'>[W]</a> up to one of your cameras ...")

		for(var/mob/O in player_list)
			if (istype(O.machine, /obj/machinery/computer/security))
				var/obj/machinery/computer/security/S = O.machine
				if (S.current == src)
					to_chat(O, "[U] holds a <a href='byond://?src=\ref[src];picturename=[rhtml_encode(W.name)]'>[W]</a> up to one of the cameras ...")
	else
		..()
	return
